...
try:
    from src.unified_team_resolver import resolver as unified_resolver
except ImportError:
...
class RobustTelegramParser:
    ...
    BET_PATTERNS = [
        ...
        re.compile(
            r"^([A-Za-z][A-Za-z\s&\'.,-]*?)\s+([-+]?\d+\.?\d*)\s+([-+]\d{3,})\s+\$(\d+)", re.I
        ),
        # "Team o/uTotal odds $amount" - e.g., "Nets o219 -112 $50"
        re.compile(
            r"^([A-Za-z][A-Za-z\s&\'.,-]*?)\s+([ou])(\d+\.?\d*)\s+([-+]\d{3,})\s+\$(\d+)", re.I
        ),
        ...
        re.compile(
            r"^([A-Za-z][A-Za-z\s&\'.,-]*?)\s+([-+]?\d+\.?\d*)\s+(1[hH]|2[hH]|[fF][gG]|1[qQ]|2[qQ]|3[qQ]|4[qQ])\s+([-+]\d{3,})",
            re.I,
        ),
    ...
    # Regex for Telegram plaintext message header:
    # "Username, [DD.MM.YYYY HH:MM]" (with optional seconds)
    ...
    def __init__(self):
        ...
        self.unified_resolver = unified_resolver

        # Build dynamic team nickname sets from team_registry for league inference
        self._nfl_teams = set(self.NFL_TEAMS)
        self._nba_teams = set(self.NBA_TEAMS)
        self._ncaaf_teams: Set[str] = set()
        self._ncaam_teams: Set[str] = set()

        # Augment from TeamRegistry aliases
        try:
            for alias, (canonical, league) in team_registry.all_aliases.items():
                lower = alias.lower()
                if league == "NFL":
                    self._nfl_teams.add(lower)
                elif league == "NBA":
                    self._nba_teams.add(lower)
                elif league == "NCAAF":
                    self._ncaaf_teams.add(lower)
                elif league == "NCAAM":
                    self._ncaam_teams.add(lower)
        except Exception:
            pass

    def parse_files(self, file_paths: List[str], date_range: Tuple[str, str] = None) -> List[Pick]:
        """Parse multiple Telegram HTML or text files."""
        all_picks = []
    ...
    def parse_file(self, file_path: str, date_range: Tuple[str, str] = None) -> List[Pick]:
        """Parse a single Telegram HTML or plain-text file (auto-detected)."""
        with open(file_path, "r", encoding="utf-8") as f:
            content = f.read()

        # Auto-detect format
        if content.lstrip().startswith(("<", "<!")) or "<html" in content[:500].lower():
            return self.parse_html(content, date_range)
        else:
            return self.parse_text(content, date_range)

    # ------------------------------------------------------------------
    # Plain-text Telegram export parsing
    # ------------------------------------------------------------------
    def parse_text(self, text_content: str, date_range: Tuple[str, str] = None) -> List[Pick]:
        """Parse a Telegram plain-text export.

        The expected format is produced by Telegram Desktop's "Export chat history"
        as plain text, or manual copy-paste from Telegram:

            Username, [DD.MM.YYYY HH:MM]
            message body (may span several lines)

        Messages appear newest-first in the file.  We reverse them so that
        conversational context (matchup, league, segment) flows forward
        chronologically, matching the HTML parser's behaviour.
        """
        # Parse date range
        ...
        # Split into (sender, timestamp, body) triplets
        raw_messages = self._split_text_messages(text_content)
        ...
        for sender, timestamp, body in raw_messages:
            # Date-range filter
            ...
            # Process every line of the body as a potential pick/context
            for line in body.splitlines():
                line = line.strip()
                if not line:
                    continue
                message_picks = self._parse_message(line)
                picks.extend(message_picks)

        return picks

    def _split_text_messages(self, text_content: str) -> List[Tuple[str, datetime, str]]:
        """Split raw text into (sender, datetime, body) tuples."""
        ...
        for raw_line in lines:
            m = self._TXT_HEADER_RE.match(raw_line.strip())
            if m:
                # Flush previous message
                if current_sender is not None and current_ts is not None:
                    messages.append((current_sender, current_ts, "\n".join(body_lines)))

                sender = m.group(1).strip()
                day, month, year = int(m.group(2)), int(m.group(3)), int(m.group(4))
                hour, minute = int(m.group(5)), int(m.group(6))
                second = int(m.group(7)) if m.group(7) else 0

                current_sender = sender
                current_ts = datetime(year, month, day, hour, minute, second)
                body_lines = []
            else:
    ...
    def _parse_slash_format(self, text: str) -> List[Pick]:
        ...
        for match in pattern.finditer(text):
            ...
            # Normalize teams via unified resolver
            t1_norm, l1 = self._resolve_team(team1.strip())
            t2_norm, l2 = self._resolve_team(team2.strip())
            pick_norm, _ = self._resolve_team(pick_team.strip())
        ...
        for part in parts:
            ...
            for pattern in self.BET_PATTERNS:
                ...
                if flex_match:
                    ...
                    # Normalize
                    team_norm, league = self._resolve_team(team)
                    ...
                    if team_norm or len(team) >= 3:
                        ...
                        if ou:
                            ...
                            pick_desc = (
                                f"{team_norm or team} +{spread}"
                                if spread
                                else f"{team_norm or team} ML"
                            )
                        ...
                        picks.append(
                            Pick(
                                ...
                                pick_description=pick_desc,
                                segment=self.SEGMENT_MAP.get(
                                    segment.lower() if segment else "fg", "FG"
                                ),
                                odds=odds,
    ...
    def _resolve_team(
        self, text: str, league_hint: str = None
    ) -> Tuple[Optional[str], Optional[str]]:
        """Resolve a team name using unified resolver first, falling back to team_registry.

        Returns (canonical_name, league) or (None, None).
        """
        hint = league_hint or self.context.current_league

        # Try unified resolver first (has the broadest alias coverage)
        if self.unified_resolver:
            canonical, league = self.unified_resolver.resolve(text, league_hint=hint)
            if canonical:
                return canonical, league

        # Fall back to team_registry
        return self.team_registry.normalize_team(text, hint)
    ...
    def _extract_matchup(self, text: str) -> bool:
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                # Normalize via unified resolver
                t1_norm, l1 = self._resolve_team(team1)
                t2_norm, l2 = self._resolve_team(team2)
    ...
    def _update_league(self, text: str):
        ...
        # Infer from team names using dynamic sets
        words = set(re.findall(r"\b\w+\b", text_lower))
        if words & self._nfl_teams:
            self.context.current_league = "NFL"
        elif words & self._nba_teams:
            self.context.current_league = "NBA"
        elif words & self._ncaaf_teams:
            self.context.current_league = "NCAAF"
        elif words & self._ncaam_teams:
            self.context.current_league = "NCAAM"

        # Fallback: use unified resolver for multi-word team mentions
        if not self.context.current_league and self.unified_resolver:
            canonical, league = self.unified_resolver.resolve(text)
            if league:
    ...
    def _create_pick_from_match(self, match: re.Match, pattern_str: str) -> Optional[Pick]:
        ...
        if groups[0].lower() in ["over", "under", "o", "u"]:
            ...
            # Normalize team
            team_norm, league = self._resolve_team(team_text, self.context.current_league)
...
def parse_telegram_directory(
    directory: str = "telegram_text_history_data", date_range: Tuple[str, str] = None
) -> List[Pick]:
    """Convenience function to parse all HTML and TXT files in directory."""
    dir_path = Path(directory)
    html_files = list(dir_path.glob("*.html"))
    txt_files = list(dir_path.glob("*.txt"))

    parser = RobustTelegramParser()
    return parser.parse_files([str(f) for f in html_files + txt_files], date_range)
