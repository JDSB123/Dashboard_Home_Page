...
class EndToEndPipeline:
    ...
    def _resolve_team(self, text: str, league_hint: str = None):
        """Resolve team name using unified resolver (preferred) or team_registry fallback."""
        if self.unified_resolver:
            canonical, league = self.unified_resolver.resolve(text, league_hint=league_hint)
            if canonical:
                return canonical, league
        return self.team_registry.normalize_team(text, league_hint)
    ...
    def _find_matching_game(self, pick: Pick) -> Optional[Dict]:
        ...
        for game in games:
            ...
            # Normalize game teams via unified resolver
            home_norm, _ = self._resolve_team(home_team, pick.league)
            away_norm, _ = self._resolve_team(away_team, pick.league)
            ...
            for candidate in candidate_teams:
                ...
                # Normalize candidate via unified resolver
                cand_norm, _ = self._resolve_team(candidate, pick.league)
            ...
            # Also try unified resolver to compare canonical forms
            if game_score < 50:
                for candidate in candidate_teams:
                    if not candidate:
                        continue
                    cand_resolved, _ = self._resolve_team(candidate, pick.league)
                    if cand_resolved:
                        home_resolved, _ = self._resolve_team(home_team, pick.league)
                        away_resolved, _ = self._resolve_team(away_team, pick.league)
                        if cand_resolved == home_resolved or cand_resolved == away_resolved:
                            game_score = max(game_score, 95)
    ...
    def export_to_excel(
        ...
        for p in picks:
            data.append(
                {
                    "Date & Time (CST)": (
                        p.date_time_cst.strftime("%m/%d/%Y %H:%M") if p.date_time_cst else ""
                    ),
                    "Date": p.date or "",
                    "League": p.league or "",
                    ...
                    "Pick (Odds)": (
                        f"{p.pick_description} ({p.odds})" if p.odds else p.pick_description or ""
                    ),
            ...
            summary_data.append(
                {
                    ...
                    "Pending": l_pending,
                    "Win Rate": (
                        f"{l_hits/(l_hits+l_misses)*100:.1f}%" if (l_hits + l_misses) > 0 else "N/A"
                    ),
            ...
            summary_data.append(
                {
                    ...
                    "Pending": s_pending,
                    "Win Rate": (
                        f"{s_hits/(s_hits+s_misses)*100:.1f}%" if (s_hits + s_misses) > 0 else "N/A"
                    ),
...
def run_pipeline(
    ...
    picks = pipeline.process_telegram_files(
        telegram_files, date_range=date_range, base_unit=base_unit
    )
