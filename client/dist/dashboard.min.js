...
(function() {
    ...
    function cleanupSampleData() {
        const CLEANUP_VERSION_KEY = 'gbsv_cleanup_v2';
        if (localStorage.getItem(CLEANUP_VERSION_KEY)) return;
        localStorage.removeItem('gbsv_demo_imported_v1');
        localStorage.removeItem('gbsv_cleanup_v1');
        const picks = getAllPicks();
        if (picks.length === 0) { localStorage.setItem(CLEANUP_VERSION_KEY, 'done'); return; }
        const realPicks = picks.filter(pick => {
            if (pick.isDemo || pick.isSample || pick.source === 'demo') return false;
            if (pick.sportsbook === 'Demo Book') return false;
            if (pick.sportsbook && pick.sportsbook !== 'unknown') return true;
            if (pick.id && pick.id.startsWith('pick_')) return true;
            return false;
        });
    ...
    function isDashboardPage() {
        if (document.body.classList.contains('page-weekly-lineup')) return false;
        const path = window.location.pathname.toLowerCase();
        if (path.includes('weekly-lineup') || path.includes('odds-market')) return false;
        return true;
    }
    function initialize() {
    ...
    console.log('✅ LocalPicksManager v33.01.0 loaded (production - no demo data)');
})();
