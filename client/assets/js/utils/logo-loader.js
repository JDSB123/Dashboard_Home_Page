...
window.LogoLoader = (() => {
  ...
  async function loadNcaamVariants() {
    try {
      ...
      // Register a key and also its space-stripped variant so slugs like
      // "centralmichigan" (generated by .replace(/\s+/g,"")) still resolve.
      const setId = (key, id) => {
        ncaamEspnIds.set(key, id);
        const stripped = key.replace(/\s+/g, "");
        if (stripped !== key) ncaamEspnIds.set(stripped, id);
      };
  ...
  function getLogoUrl(league, teamId) {
    ...
    if (
      ...
      const candidates = [
        normalizedTeamId,
        normalizedTeamId.replace(/\s+/g, ""),
        normalizedTeamId.replace(/[^a-z0-9\s]/g, ""),
        normalizedTeamId.replace(/[^a-z0-9]/g, ""),
      ];
      let espnId;
