...
(function () {
  ...
  async function init() {
    ...
    renderBookHeaders();
    bindEvents();
    // Do NOT auto-fetch — wait for user to click refresh
    renderEmptyState();
  }
  ...
  function bindEvents() {
    ...
    document.querySelectorAll("#sport-tabs .date-toggle-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        document
          .querySelectorAll("#sport-tabs .date-toggle-btn")
          .forEach((b) => b.classList.remove("active"));
        btn.classList.add("active");
  ...
  function renderBookHeaders() {
    ...
    const sharpHtml = SHARP_BOOKS.map(
      (book) => `
    ...
    const squareHtml = SQUARE_BOOKS.map(
      (book) => `
    ...
    container.innerHTML = `
            <div class="consensus-header-col">
                <span class="book-short-name">MKT</span>
                <span class="book-full-name">Market</span>
            </div>
  ...
  function getTeamInfo(teamName, league) {
    ...
    // Validate the returned entry matches the expected league to avoid cross-league collisions
    if (window.TeamData) {
      const info = window.TeamData.getTeamInfo(teamName, tdLeague);
      if (info && info.logo) {
        const infoLeague = (info.league || "").toLowerCase();
        const expectedLeague = tdLeague.toLowerCase();
        // Accept if league matches, or if TeamData didn't tag a league
        if (!infoLeague || infoLeague === expectedLeague) return info;
      }
    }
    // Fallback to static map + LogoLoader (league-safe since map keys are unique full names)
    const abbr = TEAM_ABBR_MAP[teamName];
    let logo = "";
    if (abbr && window.LogoLoader) {
      try {
        logo = window.LogoLoader.getLogoUrl(league, abbr);
      } catch {}
    }
    return {
      abbr: abbr || "",
  ...
  function transformEvent(event, sportLabel, league) {
    ...
    return {
      ...
      away: {
        ...
        abbr: TEAM_ABBR_MAP[event.away_team] || "",
      },
      home: {
        name: event.home_team,
        abbr: TEAM_ABBR_MAP[event.home_team] || "",
      },
      time: commenceTime,
      isLive,
      liveData:
        isLive && event.scores
          ? {
              away: event.scores?.[0]?.score || "",
              home: event.scores?.[1]?.score || "",
            }
          : null,
      books,
    };
  ...
  function calcConsensus(game) {
    ...
    ALL_BOOKS.forEach((book) => {
      ...
      if (!d) return;
      if (d.spread.away.line != null) {
        spreadAway.push(d.spread.away.line);
        spreadAwayOdds.push(d.spread.away.odds);
      }
      if (d.spread.home.line != null) {
        spreadHome.push(d.spread.home.line);
        spreadHomeOdds.push(d.spread.home.odds);
      }
      ...
      if (d.total.over.line != null) {
        totalOver.push(d.total.over.line);
        totalOverOdds.push(d.total.over.odds);
      }
      if (d.total.under.line != null) {
        totalUnder.push(d.total.under.line);
        totalUnderOdds.push(d.total.under.odds);
      }
    });
    ...
    cons.spread.away = {
      line: median(spreadAway),
      odds:
        avg(spreadAwayOdds) != null ? Math.round(avg(spreadAwayOdds)) : null,
    };
    cons.spread.home = {
      line: median(spreadHome),
      odds:
        avg(spreadHomeOdds) != null ? Math.round(avg(spreadHomeOdds)) : null,
    };
    cons.moneyline.away = {
      odds: avg(mlAway) != null ? Math.round(avg(mlAway)) : null,
    };
    cons.moneyline.home = {
      odds: avg(mlHome) != null ? Math.round(avg(mlHome)) : null,
    };
    cons.total.over = {
      line: median(totalOver),
      odds: avg(totalOverOdds) != null ? Math.round(avg(totalOverOdds)) : null,
    };
    cons.total.under = {
      line: median(totalUnder),
      odds:
        avg(totalUnderOdds) != null ? Math.round(avg(totalUnderOdds)) : null,
    };
  ...
  async function loadLiveOdds() {
    ...
    if (container) {
      container.innerHTML =
        '<div class="loading-odds">Loading live odds...</div>';
    }
    ...
    const results = await Promise.allSettled(
      SPORT_CONFIGS.map(async (cfg) => {
        const url =
          ...
          "/odds/" +
          cfg.apiSport +
          "/odds?markets=h2h,spreads,totals&regions=us,us2,eu";
    ...
    for (const result of results) {
      if (result.status === "fulfilled") {
        ...
        console.log(
          "[OddsMarket] " + cfg.label + ": " + events.length + " events",
        );
      } else {
        console.warn(
          "[OddsMarket] Failed:",
    ...
    if (timeEl) {
      timeEl.textContent = state.lastRefresh.toLocaleTimeString([], {
        hour: "numeric",
        minute: "2-digit",
      });
    }
  ...
  async function enrichWithRecords() {
      ...
      leagues.map(async (league) => {
        try {
          const resp = await fetch(
            `${base}/api/basketball-api/${league}/standings?season=2025`,
          );
          if (!resp.ok) return;
    ...
    for (const game of state.games) {
      ...
      if (awayRec && !game.away.record) {
        game.away.record = awayRec;
        updated = true;
      }
      if (homeRec && !game.home.record) {
        game.home.record = homeRec;
        updated = true;
      }
    }
    ...
    if (updated) {
      ...
      // Update record spans in-place instead of full re-render to avoid blinking
      document.querySelectorAll(".game-row").forEach((row) => {
        const gameId = row.dataset.gameId;
        const game = state.games.find((g) => g.id === gameId);
        if (!game) return;
        const teamLines = row.querySelectorAll(".team-line");
        if (teamLines[0] && game.away.record) {
          let recEl = teamLines[0].querySelector(".team-record");
          if (!recEl) {
            recEl = document.createElement("span");
            recEl.className = "team-record";
            const nameEl = teamLines[0].querySelector(".team-name");
            if (nameEl) nameEl.after(recEl);
          }
          recEl.textContent = `(${game.away.record})`;
        }
        if (teamLines[1] && game.home.record) {
          let recEl = teamLines[1].querySelector(".team-record");
          if (!recEl) {
            recEl = document.createElement("span");
            recEl.className = "team-record";
            const nameEl = teamLines[1].querySelector(".team-name");
            if (nameEl) nameEl.after(recEl);
          }
          recEl.textContent = `(${game.home.record})`;
        }
      });
    }
  }
  ...
  function render() {
    ...
    let games = state.games.filter((game) => {
      if (state.sport !== "all" && game.sport.toLowerCase() !== state.sport)
        return false;
    ...
    games.forEach((game) => {
      ...
      ALL_BOOKS.forEach((book) => {
        ...
        if (data.spread.away.line != null) {
          if (
            !best[game.id].spread.away ||
            data.spread.away.line > best[game.id].spread.away.line
          ) {
            best[game.id].spread.away = {
              book: book.key,
              line: data.spread.away.line,
            };
          }
        }
        if (data.spread.home.line != null) {
          if (
            !best[game.id].spread.home ||
            data.spread.home.line > best[game.id].spread.home.line
          ) {
            best[game.id].spread.home = {
              book: book.key,
              line: data.spread.home.line,
            };
          }
        }
        if (data.moneyline.away.odds != null) {
          if (
            !best[game.id].moneyline.away ||
            data.moneyline.away.odds > best[game.id].moneyline.away.odds
          ) {
            best[game.id].moneyline.away = {
              book: book.key,
              odds: data.moneyline.away.odds,
            };
          }
        }
        if (data.moneyline.home.odds != null) {
          if (
            !best[game.id].moneyline.home ||
            data.moneyline.home.odds > best[game.id].moneyline.home.odds
          ) {
            best[game.id].moneyline.home = {
              book: book.key,
              odds: data.moneyline.home.odds,
            };
          }
        }
        if (data.total.over.line != null) {
          if (
            !best[game.id].total.over ||
            data.total.over.line < best[game.id].total.over.line
          ) {
            best[game.id].total.over = {
              book: book.key,
              line: data.total.over.line,
            };
          }
        }
        if (data.total.under.line != null) {
          if (
            !best[game.id].total.under ||
            data.total.under.line > best[game.id].total.under.line
          ) {
            best[game.id].total.under = {
              book: book.key,
              line: data.total.under.line,
            };
          }
        }
      });
  ...
  function renderGameRow(game, bestOdds) {
    const timeStr = game.time.toLocaleTimeString([], {
      hour: "numeric",
      minute: "2-digit",
    });
    ...
    const awayLogoHtml = awayInfo.logo
      ? `<img class="team-logo" src="${awayInfo.logo}" alt="" loading="lazy" onerror="this.style.display='none'">`
      : "";
    const homeLogoHtml = homeInfo.logo
      ? `<img class="team-logo" src="${homeInfo.logo}" alt="" loading="lazy" onerror="this.style.display='none'">`
      : "";
    ...
    const awayRecordHtml = awayRecord
      ? `<span class="team-record">(${awayRecord})</span>`
      : "";
    const homeRecordHtml = homeRecord
    ...
    const awayScore =
      game.isLive && game.liveData
        ? `<span class="team-score">${game.liveData.away}</span>`
        : "";
    const homeScore =
      game.isLive && game.liveData
        ? `<span class="team-score">${game.liveData.home}</span>`
        : "";
    ...
    if (state.market === "spread") {
      const vig = calcVig(
        consensus.spread.away.odds,
        consensus.spread.home.odds,
      );
      vigHtml =
        vig != null
          ? `<span class="vig-display">${vig.toFixed(1)}%</span>`
          : "";
    } else if (state.market === "moneyline") {
      const vig = calcVig(
        consensus.moneyline.away.odds,
        consensus.moneyline.home.odds,
      );
      vigHtml =
        vig != null
          ? `<span class="vig-display">${vig.toFixed(1)}%</span>`
      ...
      const vig = calcVig(
        consensus.total.over.odds,
        consensus.total.under.odds,
      );
      vigHtml =
        vig != null
          ? `<span class="vig-display">${vig.toFixed(1)}%</span>`
    ...
    const sharpHtml = SHARP_BOOKS.map((book) =>
      renderBookCol(game, book, bestOdds, "sharp"),
    ).join("");
    // Square books columns
    const squareHtml = SQUARE_BOOKS.map((book) =>
      renderBookCol(game, book, bestOdds, "square"),
    ).join("");
  ...
  function renderConsensusCol(game, consensus) {
    ...
    if (state.market === "spread") {
      awayData = {
        line: formatLine(consensus.spread.away.line),
        odds: formatOdds(consensus.spread.away.odds),
      };
      homeData = {
        line: formatLine(consensus.spread.home.line),
        odds: formatOdds(consensus.spread.home.odds),
      };
    } else if (state.market === "moneyline") {
      ...
      awayData = {
        line:
          "O " +
          (consensus.total.over.line != null
            ? consensus.total.over.line
            : "N/A"),
        odds: formatOdds(consensus.total.over.odds),
      };
      homeData = {
        line:
          "U " +
          (consensus.total.under.line != null
            ? consensus.total.under.line
            : "N/A"),
        odds: formatOdds(consensus.total.under.odds),
      };
    }
  ...
  function renderOddsCell(game, book, data) {
    return `
            <div class="odds-cell ${data.isBest ? "best-odds" : ""}">
  ...
  function updateKPIs() {
    ...
    const filteredGames = state.games.filter((g) => {
      if (state.sport !== "all" && g.sport.toLowerCase() !== state.sport)
        return false;
    ...
    if (liveEl)
      liveEl.textContent = filteredGames.filter((g) => g.isLive).length;
