...
(function() {
    ...
    // ===== FILTER STATE =====
    let activeFilters = {
        datetime: { range: 'today', timeSlots: ['morning', 'early', 'afternoon', 'evening', 'primetime', 'late'] },
        ...
        return `
            <div class="odds-section fg-section">
                ${renderOddsGroup('Spread', [
                    renderOddsRow(game.awayTeam.abbr, [renderOddsBtn(game.id, 'spread', 'away', 'fg', fg.spread.away.line, fg.spread.away.odds)]),
                    renderOddsRow(game.homeTeam.abbr, [renderOddsBtn(game.id, 'spread', 'home', 'fg', fg.spread.home.line, fg.spread.home.odds)])
                ])}
                ${renderOddsGroup('Moneyline', [
                    renderOddsRow(game.awayTeam.abbr, [renderMlBtn(game.id, 'ml', 'away', 'fg', fg.ml.away)]),
                    renderOddsRow(game.homeTeam.abbr, [renderMlBtn(game.id, 'ml', 'home', 'fg', fg.ml.home)])
                ])}
                ${renderOddsGroup('Total O/U', [
                    renderOddsRow('O', [renderOddsBtn(game.id, 'total', 'over', 'fg', fg.total.line, fg.total.over)]),
                    renderOddsRow('U', [renderOddsBtn(game.id, 'total', 'under', 'fg', fg.total.line, fg.total.under)])
                ])}
        ...
        return `
            <div class="odds-section half-section">
                ${renderOddsGroup('Spread 1H', [
                    renderOddsRow(game.awayTeam.abbr, [renderOddsBtn(game.id, 'spread', 'away', '1h', h1.spread.away.line, h1.spread.away.odds)]),
                    renderOddsRow(game.homeTeam.abbr, [renderOddsBtn(game.id, 'spread', 'home', '1h', h1.spread.home.line, h1.spread.home.odds)])
                ])}
                ${renderOddsGroup('Moneyline 1H', [
                    renderOddsRow(game.awayTeam.abbr, [renderMlBtn(game.id, 'ml', 'away', '1h', h1.ml.away)]),
                    renderOddsRow(game.homeTeam.abbr, [renderMlBtn(game.id, 'ml', 'home', '1h', h1.ml.home)])
                ])}
                ${renderOddsGroup('Total O/U 1H', [
                    renderOddsRow('O', [renderOddsBtn(game.id, 'total', 'over', '1h', h1.total.line, h1.total.over)]),
                    renderOddsRow('U', [renderOddsBtn(game.id, 'total', 'under', '1h', h1.total.line, h1.total.under)])
                ])}
            </div>
        `;
    }
    ...
    function renderTeamTotalsGroup(game, tt) {
        if (!tt) {
            return renderOddsGroup('Team Total', [
                renderOddsRow('TT', ['<span class="odds-placeholder">No team totals</span>'])
            ]);
        }
    ...
    function getTeamLogoUrl(abbr, league) {
        if (window.LogoCache && typeof window.LogoCache.generateLogoUrl === 'function') {
            return window.LogoCache.generateLogoUrl(abbr, league);
        }
        const leagueMap = { nfl: 'nfl', nba: 'nba', mlb: 'mlb', nhl: 'nhl', ncaab: 'ncaa', ncaaf: 'ncaa' };
    ...
    function bindExpandButtons() {
        document.querySelectorAll('.expand-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                expandedGames.has(gameId) ? expandedGames.delete(gameId) : expandedGames.add(gameId);
                renderGames();
            });
        });
    }
    ...
    function handleOddsClick(btn) {
        ...
        if (existingIndex >= 0) {
            ...
            let pickTeam = side === 'away' ? game.awayTeam.name : side === 'home' ? game.homeTeam.name : null;
    ...
    function renderBetslip() {
        ...
        if (betslip.length === 0) {
            body.innerHTML = '<div class="betslip-empty"><p>Click any odds to add</p></div>';
        ...
        body.querySelectorAll('.betslip-remove').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                const oddsBtn = document.querySelector(`[data-pick-id="${removed.id}"]`);
    ...
    function formatPickLabel(pick) {
        switch (pick.type) {
            case 'spread': return `${pick.pickTeam} ${pick.line}`;
            case 'total': return `${pick.side === 'over' ? 'Over' : 'Under'} ${pick.line}`;
            case 'ml': return `${pick.pickTeam} ML`;
            case 'teamTotal': return pick.line;
            default: return 'Pick';
        }
    }
    ...
    function updateTotals() {
        ...
        if (totalWinEl) totalWinEl.textContent = `$${Math.round(totalWin).toLocaleString()}`;
    }
    ...
    function placeBets() {
        ...
        const picks = betslip.map(pick => ({
            pickTeam: pick.pickTeam || (pick.side === 'over' ? 'Over' : pick.side === 'under' ? 'Under' : pick.line),
            pickType: pick.type,
            line: pick.line,
            odds: pick.odds,
        ...
        if (window.LocalPicksManager) {
            ...
            showNotification(`${picks.length} pick(s) added to Dashboard!`, 'success');
        } else {
    ...
    function updateKPIs() {
        ...
        if (liveCount) liveCount.textContent = filtered.filter(g => g.isLive).length;
        ...
        document.querySelectorAll('.filter-action-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (action === 'select-all') {
                    dropdown.querySelectorAll('.time-slot').forEach(s => s.classList.add('active'));
                    dropdown.querySelectorAll('.league-pill').forEach(p => {
                        p.classList.remove('active');
                        if (p.dataset.league === 'all') p.classList.add('active');
                    });
                    activeFilters.datetime.timeSlots = ['morning', 'early', 'afternoon', 'evening', 'primetime', 'late'];
                    activeFilters.matchup.league = 'all';
                } else if (action === 'clear') {
                    dropdown.querySelectorAll('.time-slot').forEach(s => s.classList.remove('active'));
        ...
        sorted.sort((a, b) => {
            ...
            switch (currentSort.column) {
                ...
                case '1h':
                    ...
                    valA = a.odds['1h'] ? parseFloat(a.odds['1h'].spread.home.line) || 0 : 999;
                    valB = b.odds['1h'] ? parseFloat(b.odds['1h'].spread.home.line) || 0 : 999;
                    break;
                default:
                    valA = 0;
                    valB = 0;
