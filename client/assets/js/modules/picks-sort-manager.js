...
(function () {
    ...
    getSortValue(row, column) {
      switch (column) {
        case "date": {
          ...
          const dateValue = this.getDateSortValue(
            dateEl ? dateEl.textContent : "",
          );
          const timeValue = this.getTimeSortValue(
        ...
        case "status": {
          const cell =
            this.getCell(row, "status") || row.querySelector("td:last-child");
          if (!cell) return 0;
          ...
          return this.getStatusSortPriority(
  ...
  // Auto-initialize sorting for dashboard table headers.
