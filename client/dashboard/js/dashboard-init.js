  ...
  const DashboardInit = {
    ...
    initKPITiles() {
      const tiles = document.querySelectorAll(
        '.kpi-tile[data-action="flip-tile"]',
      );
    ...
    async handleRefresh(btn) {
      ...
      try {
        // Always prefer a true data refresh path first; reEnrich is metadata-only.
        if (typeof window.loadLivePicks === "function") {
          await window.loadLivePicks();
        } else if (
          window.ActivePicks &&
          typeof window.ActivePicks.refreshData === "function"
        ) {
          await window.ActivePicks.refreshData();
        } else if (
        ...
        // Optionally backfill missing record metadata after live refresh.
        if (
          window.LocalPicksManager &&
          typeof window.LocalPicksManager.reEnrich === "function"
        ) {
