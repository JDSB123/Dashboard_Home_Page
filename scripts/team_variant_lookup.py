...
class TeamVariantLookup:
    ...
    def __init__(self, data_dir: str = "assets/data/team-variants"):
        ...
        self.cfb_teams = self._load_json('cfb_team_variants.json') if (self.data_dir / 'cfb_team_variants.json').exists() else {}
    ...
    def find_ncaam_team(self, query: str) -> Optional[Dict]:
        ...
        for team_key, team_data in self.ncaam_teams.items():
            ...
            if (query_upper in name or 
                query_upper in location or 
                query_upper in nickname or
                query_upper == location.split()[0] if location else False):  # First word of location
                return {'key': team_key, **team_data}
