...
const MAPPINGS_PATH = path.join(ROOT, "client", "assets", "data", "logo-mappings.json");
const VARIANTS_DIR  = path.join(ROOT, "client", "assets", "data", "team-variants");
...
function log(msg, color="reset") {
  const c = { reset:"\x1b[0m", cyan:"\x1b[36m", green:"\x1b[32m",
...
function azUpload(localFile, blobName) {
...
// auth-mode login uses the current `az login` session — no key needed.
...
if (ALL || onlyLeagues.has("nba")) {
  for (const [abbr, t] of Object.entries(nbaVariants)) {
    ...
    const blobId = existingMappings.logoMappings?.nba?.[primaryAbbr]
      ?.replace("nba-500-","").replace(".png","") || primaryAbbr;
    manifest.push({
...
if (ALL || onlyLeagues.has("nfl")) {
  for (const [abbr, t] of Object.entries(nflVariants)) {
    ...
    const blobId = existingMappings.logoMappings?.nfl?.[primaryAbbr]
      ?.replace("nfl-500-","").replace(".png","") || primaryAbbr;
    manifest.push({
...
if (ALL || onlyLeagues.has("nhl")) {
  ...
  const nhlTeamConfig = JSON.parse(
    fs.readFileSync(path.join(ROOT, "client/assets/data/team-config.json"), "utf8")
  ).nhl?.teams || {};
...
log("\n╔══════════════════════════════════════════════════════════════════╗", "cyan");
log("║           GBSV TEAM LOGO INGESTION PIPELINE                     ║", "cyan");
log("╚══════════════════════════════════════════════════════════════════╝\n", "cyan");
...
// ─── Step 1: Download ──────────────────────────────────────────────────────
log("\n── Step 1: Downloading from ESPN CDN ─────────────────────────────", "cyan");
...
const tasks = deduped.map(entry => async () => {
  ...
  try {
    ...
    if (r.cached) { cached++; process.stdout.write("."); }
    else          { downloaded++; process.stdout.write("+"); }
  } catch(e) {
    failed++;
    failedList.push({ name: entry.blobName, url: entry.url, error: e.message });
...
if (failed > 0) {
  ...
  failedList.slice(0,10).forEach(f => log(`    ✗ ${f.name}: ${f.error}`, "red"));
  if (failedList.length > 10) log(`    ... and ${failedList.length-10} more`, "red");
}

// ─── Step 2: Upload to blob storage ───────────────────────────────────────
if (!SKIP_UPLOAD) {
  log("\n── Step 2: Uploading to Azure Blob Storage ───────────────────────", "cyan");
  ...
  if (!DRY_RUN) {
    log(`  Uploading ${fs.readdirSync(ARTIFACTS).filter(f=>f.endsWith(".png")).length} files from ${ARTIFACTS}...`, "gray");
    const r = spawnSync("az", [
    ...
    if (r.status === 0) {
      uploaded = fs.readdirSync(ARTIFACTS).filter(f=>f.endsWith(".png")).length;
      ...
      for (const entry of deduped) {
        ...
        const r2 = azUpload(localFile, entry.blobName);
        if (r2.ok) { uploaded++; process.stdout.write("."); }
        else { uploadFailed++; process.stdout.write("✗"); }
      }
      console.log();
    }
  } else {
  ...
  log(`  Uploaded: ${uploaded}  Failed: ${uploadFailed}`, uploadFailed > 0 ? "red" : "green");
}

// ─── Step 3: Regenerate logo-mappings.json ────────────────────────────────
log("\n── Step 3: Rebuilding logo-mappings.json ─────────────────────────", "cyan");
...
for (const entry of deduped) {
  ...
  if (!newMappings.logoMappings[leagueKey]) newMappings.logoMappings[leagueKey] = {};
...
if (!DRY_RUN) {
  fs.writeFileSync(MAPPINGS_PATH, JSON.stringify(newMappings, null, 2) + "\n", "utf8");
  const entryCount = Object.values(newMappings.logoMappings)
    .reduce((s, v) => s + Object.keys(v).length, 0);
  log(`  ✓ logo-mappings.json updated — ${entryCount} total lookup entries`, "green");
  for (const [league, entries] of Object.entries(newMappings.logoMappings)) {
    log(`    ${league.padEnd(8)}: ${Object.keys(entries).length} entries`, "gray");
  }
} else {
...
log("\n── Step 4: Writing artifact index ───────────────────────────────", "cyan");
const indexData = {
...
log(`  Blob storage:    https://${STORAGE_ACCOUNT}.blob.core.windows.net/${CONTAINER}`, "gray");
log(`  Mappings index:  ${MAPPINGS_PATH}\n`, "gray");

if (failedList.length > 0) {
    log(`⚠  ${failedList.length} logo(s) could not be downloaded from ESPN CDN`, "yellow");
    log("   Re-run to retry, or check ESPN CDN availability for those teams.\n", "yellow");
    process.exit(1);
  }
} // end main()
