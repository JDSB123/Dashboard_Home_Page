...
def run_calculation():
    ...
    for idx, row in df_filtered.iterrows():
        ...
        try:
            ...
            if games:
                ...
                if game:
                    ...
                    if pick_info['type'] != 'total':
                        ...
                        # No need to check away explicitly for boolean logic, just assuming opposite if not home found? 
                        # Better to be safe, but for simple PnL logic above, we usually filtered it.
                        # Using logic from regenerate_analysis.py which checked both.
                    ...
                    if pick_info['type'] == 'total':
                        ...
                        # Simplistic home/away detection from earlier.
                        # If pick_team_is_home check is robust enough. If we didn't match home, assume pick is away? 
                        # Or should we check away explicitly?
                        # Re-implementing explicitly to be sure
            ...
            if 'Hawks' in str(row['Matchup']):
                print(f"Error on Hawks: {e}")
            pass
        
        if grade == 'Win': wins += 1
        elif grade == 'Loss': losses +=1
        elif grade == 'Push': pushes += 1
        else:
            pending += 1
            if pending <= 10:
                print(f"PENDING: {date_str} {league} {row['Matchup']} - {row['Pick']}")

        total_pnl += pnl

    print("\n------------------------------------------------")
    print(f"PnL since {start_date.date()}: ${total_pnl:,.2f}")
    print(f"Record: {wins}-{losses}-{pushes} (W-L-P) with {pending} pending/unknown")
    
    # Breakdown of pending by league
    pending_leagues = df_filtered[df_filtered.index.isin([i for i, r in df_filtered.iterrows() if 'Hit/Miss' not in r])]['League'].value_counts()
    # Wait, I don't have 'Hit/Miss' in the dataframe in this script because I iterate.
