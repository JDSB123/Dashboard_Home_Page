...
const { buildCorsHeaders, getAllowedOrigins, validateOrigin } = require("../shared/http");
...
module.exports = async function (context, req) {
  ...
  try {
    ...
    if (req.method === "POST" && (!action || actionLower === "")) {
      ...
      if (!originCheck.ok) {
        send(context, 403, { success: false, error: originCheck.reason }, corsHeaders);
        return;
      }
      ...
      if (!id) {
        send(
          context,
          400,
          { success: false, error: "Archive payload must include id" },
          corsHeaders
        );
        return;
      }
