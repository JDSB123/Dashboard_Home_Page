...
class CosmosPicksClient:
    ...
    def __init__(self):
        self.endpoint = os.getenv("COSMOS_ENDPOINT", "https://gbsv-picks-db.documents.azure.com:443/")
    ...
    def query_picks(self, league: str, season: Optional[int] = None, **filters) -> List[Dict]:
        ...
        results = list(picks_container.query_items(
            query=query,
            parameters=[],
            partition_key=league
        ))
    ...
    def get_metrics(self, league: str, season: Optional[int] = None) -> Optional[Dict]:
        ...
        results = list(metrics_container.query_items(
            query=query,
            parameters=[],
            partition_key=league
        ))
